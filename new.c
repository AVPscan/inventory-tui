...
int ccou=37, ccol=34, cpri=36, cnam=35, cnor=30, ctxt=32;
...
char* GetRow(int num, int price, int col, const char *name, int mode) {
    ...
    switch (mode) {
        ...
        case 2: sprintf(d, "%s%02d %s%s %s%*d %s%-*s%s", 
                    SetColor(ccou), num, 
                    SetColor(ccol), col_out,
                    SetColor(cpri), max_n_chars, price,
                    SetColor(cnam), name_w, name, 
                    SetColor(cnor)); break;
...
int main() {
    ...
    dict_cap = 0;
    char nname[128] = {0};
    int num,pos,ncol,nprice;
    SWD();
    if (AutoEncryptOrValidate(filesendID)) { printf("Подготовьте файл для почтовых отправлений."); return 0; }
    SetInputMode(1);
    printf("%s",HideCursor);
    num=LoadDic(DB_NAME);
    while (1) {
    ...
    if (num!=0) { printf("\n           Сумма %d",num); fflush(stdout); }
    pos=ExportDict(DB_NAME);
    pos=ExportDict(fileres);
    pos=TxtToHtml(fileres, filesendhtml, filesendID);
    pos=SendMailSecure(filesendID,filesendhtml);
    unlink(filesendhtml);
